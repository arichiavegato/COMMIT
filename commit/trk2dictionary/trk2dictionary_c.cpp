...
)
{
    ...
    printf( "\n   \033[0;32m* Exporting IC and EC compartments:\033[0m\n" );


    // Calling to the core function
    for( int i = 0; i<threads_count; i++ ){
        threads.push_back( thread( T2D, fpTractogram, isTRK, n_scalars, n_properties, ptrToVOXMM, nReplicas, ptrBlurRho, ptrBlurAngle, ptrBlurWeights,
...
FILE* pDict_EC_v, FILE* pDict_EC_o, short* ptrHashTable, vector<Vector<double>>& P  )
{
    ...
    while( !fpTractogram.eof() )  //for(int f=0; f<n_count; f++) 
    {
        ...
        if ( isTRK )
            N = read_fiberTRK( fpTractogram, fiber, n_scalars, n_properties, P );
        else
            N = read_fiberTCK( fpTractogram, fiber , ptrToVOXMM, P );
