...
std::mutex reading_mutex; // Mutex to manage the access to the file
*/ 
...
{
    // First of all -> set the global variables defined above. The values are provided by the cython script
    dim.Set( Nx, Ny, Nz );
    pixdim.Set( Px, Py, Pz );
    ...
    // Creazione e apertura dei file di output 
    ...
    // Iterate over streamlines -> This part needs to be parallelized
    ...
    // Creazione ed apertura dei file di output
